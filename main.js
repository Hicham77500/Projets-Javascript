...
const setStorage = () => localStorage.setItem('TASKS', JSON.stringify(Array.from(taskss)))
// Ajout d'une tâche
document.querySelector('input').addEventListener('keydown', e => {
    if(e.key === 'Enter')  {
      // On ajoute la tâche dans le dictionnaire
      taskss.set(e.target.value, false)  
...
document.querySelector('table').addEventListener('click', e => {
    ...
    if(e.target.matches('button')) {
      ...
      if(e.target.matches('.btn-danger')) {
        ...
        taskss.delete(e.target.parentNode.previousSibling.textContent)
